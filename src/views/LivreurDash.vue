<template>
  ...
  >
    ...
    <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
      ...
      <tbody>
        <tr
          v-for="(livreur, index) in filteredLivreurs"
          :key="index"
        ...
        >
          ...
          <td class="flex items-center px-6 py-4 space-x-3">
            <a
              ...
              href="#"
              @click="showModalLivreurUpdate = true"
            >
    ...
    async getLivreur() {
      try {
        ...
        if (response.data) {
          ...
          this.filteredLivreurs = this.livreurs.filter(
            (livreur) => livreur.user.id === this.user
          );
          console.log(this.filteredLivreurs);
        }
      } catch (error) {
        console.log(error.data);
      }
